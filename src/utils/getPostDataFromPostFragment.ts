...
const GET_CUSTOM_POST_DATA = gql`
  query GetPostByURI($postURI: String!) {
  postBy(slug: $postURI) {
    id
    title
    content
...
export const CUSTOM_POST_DATA = (postURI: any) => {
  const { loading, error, data } = useQuery(GET_CUSTOM_POST_DATA, {
    variables: {
      postURI: postURI
    }
  });
